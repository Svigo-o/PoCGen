...
- 统一输出：`output/poc`（PoC）与 `output/cookie`（浏览器 Cookie/请求响应）。
...
生成的 PoC 以时间戳命名写入 `output/poc`（如 `poc_20250101_120000_01.http`），重名自动加序号。
...
| `POCGEN_OUTPUT_DIR` | PoC 输出目录 | `${OUTPUT_ROOT}/poc` |
