...
def save_messages(raw_messages: List[str], output_dir: str, prefix: str = "poc") -> List[str]:
    os.makedirs(output_dir, exist_ok=True)
    ts = time.strftime("%Y%m%d_%H%M%S")
    paths: List[str] = []
    for idx, raw in enumerate(raw_messages, start=1):
        fname = f"{prefix}_{ts}_{idx:02d}.http"
        fpath = os.path.join(output_dir, fname)
        counter = 1
        while os.path.exists(fpath):
            fname = f"{prefix}_{ts}_{idx:02d}_{counter}.http"
            fpath = os.path.join(output_dir, fname)
            counter += 1
        cleaned = _strip_markdown_fences(raw)
        normalized = _adjust_content_length(cleaned)
