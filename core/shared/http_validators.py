...
def validate_http_message(msg: HTTPMessage, fix_content_length: bool = False) -> List[str]:
    errors: List[str] = []
    ...
    if fix_content_length:
        _fix_content_length(msg)
    elif "Content-Length" in msg.headers:
        try:
...
def _fix_content_length(msg: HTTPMessage) -> None:
...
def parse_and_validate(raw: str, fix_content_length: bool = False) -> Tuple[HTTPMessage, List[str]]:
    msg = HTTPMessage.parse(raw)
    return msg, validate_http_message(msg, fix_content_length=fix_content_length)
