...
def _build_system_prompt(attacker_url: str) -> str:
    ...
    "If a sample HTTP interaction is provided, treat it as the primary blueprint—mirror its method, path, "
    "critical headers, parameter naming, and encoding, only changing what is necessary to deliver the payload.\n"
    "If validation feedback from previous attempts is provided, you MUST fix the highlighted issues "
    "(e.g., wrong parameter, missing cookie, incorrect length) before responding.\n"
    "When refining after a failure, revisit the BODY format first—match parameter names, required fields, "
    "encoding style (JSON, form-urlencoded, plaintext, multipart), and payload syntax from the vulnerable "
    "code before adjusting headers or ancillary metadata. Mirror any command-injection delimiters such as "
    "'$(...)', '$(+ ...)', ';', '&&', or backticks that the backend recognizes.\n"
    ...
    "- Derive the HTTP method and media type from the sample or code. If JSON, send a compact JSON body; "
    "if form data, keep it URL-encoded; if plaintext or multipart, follow that format exactly. Only fall back "
    "to POST with 'application/x-www-form-urlencoded; charset=UTF-8' when the handler's expectations are "
    "unknown.\n"
    ...
    "- If no sample interaction is available, fall back to a typical structure like the following (adjust as needed):\n"
